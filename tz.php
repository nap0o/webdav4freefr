...
function GetCoreInformation() {$data = @file('/proc/stat'); if($data === false) return array();$cores = array();foreach( $data as $line ) {if( preg_match('/^cpu[0-9]/', $line) ){$info = explode(' ', $line);$cores[]=array('user'=>$info[1],'nice'=>$info[2],'sys' => $info[3],'idle'=>$info[4],'iowait'=>$info[5],'irq' => $info[6],'softirq' => $info[7]);}}return $cores;}
function GetCpuPercentages($stat1, $stat2) {if(empty($stat1) || empty($stat2) || count($stat1)!==count($stat2)){return array();}$cpus=array();for( $i = 0, $l = count($stat1); $i < $l; $i++) {	$dif = array();	$dif['user'] = $stat2[$i]['user'] - $stat1[$i]['user'];$dif['nice'] = $stat2[$i]['nice'] - $stat1[$i]['nice'];	$dif['sys'] = $stat2[$i]['sys'] - $stat1[$i]['sys'];$dif['idle'] = $stat2[$i]['idle'] - $stat1[$i]['idle'];$dif['iowait'] = $stat2[$i]['iowait'] - $stat1[$i]['iowait'];$dif['irq'] = $stat2[$i]['irq'] - $stat1[$i]['irq'];$dif['softirq'] = $stat2[$i]['softirq'] - $stat1[$i]['softirq'];$total = array_sum($dif);$cpu = array();foreach($dif as $x=>$y) $cpu[$x] = round($y / $total * 100, 2);$cpus['cpu' . $i] = $cpu;}return $cpus;}
$stat1 = GetCoreInformation();sleep(1);$stat2 = GetCoreInformation();$data = GetCpuPercentages($stat1, $stat2);
$cpu_show = isset($data['cpu0']) ? $data['cpu0']['user']."%us,  ".$data['cpu0']['sys']."%sy,  ".$data['cpu0']['nice']."%ni, ".$data['cpu0']['idle']."%id,  ".$data['cpu0']['iowait']."%wa,  ".$data['cpu0']['irq']."%irq,  ".$data['cpu0']['softirq']."%softirq" : "";
...
// 根据不同系统取得CPU相关信息
$sysInfo = array('uptime'=>'', 'win_n'=>'', 'cpu'=>array('num'=>0, 'model'=>''), 'loadAvg'=>'', 'memTotal'=>0, 'memUsed'=>0, 'memFree'=>0, 'memCached'=>0, 'memBuffers'=>0, 'swapTotal'=>0, 'swapUsed'=>0, 'swapFree'=>0, 'swapPercent'=>0, 'memRealUsed'=>0, 'memRealFree'=>0, 'memRealPercent'=>0, 'memPercent'=>0, 'memCachedPercent'=>0);
switch(PHP_OS)
{
	case "Linux":
		$tmp = sys_linux(); $sysReShow = ($tmp !== false) ? "show" : "none"; if($tmp !== false) $sysInfo = array_merge($sysInfo, $tmp);
	break;
	case "FreeBSD":
		$tmp = sys_freebsd(); $sysReShow = ($tmp !== false) ? "show" : "none"; if($tmp !== false) $sysInfo = array_merge($sysInfo, $tmp);
	break;
/*	
	case "WINNT":
		$tmp = sys_windows(); $sysReShow = ($tmp !== false) ? "show" : "none"; if($tmp !== false) $sysInfo = array_merge($sysInfo, $tmp);
	break;
*/	
	default:
	break;
...
th{padding: 3px 6px; font-weight:bold;background:#dedede;color:#626262;border:1px solid #cccccc; text-align:left; word-break: break-all;}
tr{padding: 0; background:#FFFFFF;}
td{padding: 3px 6px; border:1px solid #CCCCCC; word-break: break-all;}
...
#page {width: 100%; max-width: 960px; padding: 0 5px; box-sizing: border-box; margin: 0 auto; text-align: left;}
...
<table>
  ...
  <tr>
    ...
    <td width="37%"><?php $os = explode(" ", php_uname()); echo $os[0];?> &nbsp;内核版本：<?php $kr = php_uname('r'); echo ($kr && $kr !== 'GNU/Linux') ? $kr : (('/'==DIRECTORY_SEPARATOR) ? ($os[2] ?? '') : ($os[1] ?? '')); ?></td>
  ...
  <tr>
	  <td>服务器主机名</td>
	  <td><?php $hn = function_exists('gethostname') ? @gethostname() : ''; echo ($hn && $hn !== 'GNU/Linux' && $hn !== 'localhost') ? $hn : (isset($os[1]) ? $os[1] : php_uname('n')); ?></td>
	  <td>绝对路径</td>
	  <td><?php echo $_SERVER['DOCUMENT_ROOT']?str_replace('\\','/',$_SERVER['DOCUMENT_ROOT']):str_replace('\\','/',dirname(__FILE__));?></td>
	</tr>
  <tr>
	  <td>管理员邮箱</td>
	  <td><?php echo $_SERVER['SERVER_ADMIN'] ?? '未配置';?></td>
		<td>探针路径</td>
...
<table>
  ...
  <tr>
    <td>CPU使用状况</td>
    <td colspan="5"><?php if('/'==DIRECTORY_SEPARATOR){echo $cpu_show." | <a href='".$_SERVER['PHP_SELF']."?act=cpu_percentage' target='_blank' class='static'>查看图表</a>";}else{echo "暂时只支持Linux系统";}?>
	</td>
  </tr>
  <tr>
    <td>硬盘使用状况</td>
...
<table>
  ...
  <tr>
    ...
    <td width="18%">
		...
		$disFuns=get_cfg_var("disable_functions");
		?>
    <?php echo (false!==stripos($disFuns ?? '', 'phpinfo'))? '<font color="red">×</font>' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?>
    </td>
...
?>
    ...
    <td width="18%"><?php if(version_compare(PHP_VERSION, "5.3.0", ">=")){echo (get_cfg_var("zend_loader.enable"))?'<font color=green>√</font>':'<font color=red>×</font>';} else{if(function_exists('zend_optimizer_version')){	echo zend_optimizer_version();}else{	echo (get_cfg_var("zend_optimizer.optimization_level")||get_cfg_var("zend_extension_manager.optimizer_ts")||get_cfg_var("zend.ze1_compatibility_mode")||get_cfg_var("zend_extension_ts"))?'<font color=green>√</font>':'<font color=red>×</font>';}}?></td>
  </tr>
  <tr>
    <td>eAccelerator</td>
...
<table>
  ...
  <tr>
    ...
    <td>mSQL 数据库：</td>
    <td><?php echo isfun("class_exists('SQLite3') ? 'sqlsrv_close' : 'sqlsrv_close'");?></td>
  </tr>
  <tr>
    <td>SQLite 数据库：</td>
    <td><?php if(extension_loaded('sqlite3')) {$sqliteVer = SQLite3::version();echo '<font color=green>√</font>　';echo "SQLite3　Ver ";echo $sqliteVer['versionString'];}else {echo (extension_loaded("sqlite3") ? '<font color="green">√</font>' : '<font color="red">×</font>');if((extension_loaded("sqlite3") ? '<font color="green">√</font>' : '<font color="red">×</font>') == '<font color="green">√</font>') {echo "&nbsp; 版本： ".@sqlite_libversion();}}?></td>
    <td>Hyperwave 数据库：</td>
...
<table>
  ...
  <tr align="center">
    ...
    <td><?php echo isset($sysInfo['cpu']['model']) ? $sysInfo['cpu']['model'] : '未获取到';?></td>
  </tr>
</table>
